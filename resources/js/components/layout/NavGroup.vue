<template>
    ...
    <button v-if="collapsed" :title="label" :class="[
        'nav-collapsed-btn',
    ...
    <template v-else>
        <button @click="open = !open" :class="['nav-group-trigger', isChildActive ? 'is-active' : '']">
...
.nav-collapsed-btn {
    @apply w-full flex items-center justify-center px-3 py-2 rounded-lg text-slate-400 transition-all duration-200 hover:bg-slate-800/80 hover:text-slate-100;
}
...
.nav-group-trigger {
    @apply w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-slate-400 transition-all duration-200 hover:bg-slate-800/70 hover:text-slate-100;
}

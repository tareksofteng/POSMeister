    ...
    <div class="ledger-page">
        <header class="ledger-header print:hidden">
            <div>
                ...
                <p class="text-sm text-slate-500">{{ t('customerLedger.subtitle') }}</p>
            </div>
        ...
        <section class="filter-bar print:hidden">
            <div class="field">
                <label>{{ t('customerLedger.customer') }}</label>
                <select v-model.number="form.customer_id" class="control">
                    <option :value="null">{{ t('customerLedger.selectCustomer') }}</option>
            ...
            <button @click="generate" :disabled="loading || !form.customer_id" class="btn-primary self-end">
                <ArrowPathIcon v-if="loading" class="w-4 h-4 animate-spin" />
                <DocumentMagnifyingGlassIcon v-else class="w-4 h-4" />
                {{ loading ? t('common.loading') : t('customerLedger.generate') }}
            </button>
        </section>
        ...
        <div v-if="!report && !loading" class="empty-state print:hidden">
            ...
            <p class="text-sm text-slate-500">{{ t('customerLedger.empty') }}</p>
        </div>

        <article v-if="report" class="ledger-paper" id="ledger-paper">
            <div class="paper-head">
                <div>
                    <p class="paper-eyebrow">{{ t('customerLedger.documentTitle') }}</p>
                    <h2>{{ report.customer.name }}</h2>
                    <p class="text-xs text-slate-500" v-if="report.customer.code">{{ report.customer.code }}</p>
                    <p class="text-xs text-slate-500 mt-2" v-if="report.customer.address">{{ report.customer.address }}</p>
                    <p class="text-xs text-slate-500" v-if="report.customer.phone">{{ report.customer.phone }}</p>
                </div>
                <div class="text-right">
                    <p class="paper-eyebrow">{{ t('customerLedger.period') }}</p>
            ...
            <div class="summary-grid">
                <div class="summary-card">
                    <span class="label">{{ t('customerLedger.opening') }}</span>
                    <span class="value">{{ fmtCurrency(report.opening) }}</span>
                </div>
                <div class="summary-card">
                    <span class="label">{{ t('customerLedger.totalDebit') }}</span>
                    <span class="value text-slate-700">{{ fmtCurrency(report.totals.debit) }}</span>
                </div>
                <div class="summary-card">
                    <span class="label">{{ t('customerLedger.totalCredit') }}</span>
                    <span class="value text-emerald-700">{{ fmtCurrency(report.totals.credit) }}</span>
                </div>
                <div class="summary-card highlight">
                    <span class="label">{{ t('customerLedger.closing') }}</span>
            ...
            <table class="ledger-table">
                <thead>
                    <tr>
                        ...
                        <th class="w-28">{{ t('customerLedger.type') }}</th>
                        <th class="w-36">{{ t('customerLedger.reference') }}</th>
                        <th>{{ t('customerLedger.description') }}</th>
                        <th class="num">{{ t('customerLedger.debit') }}</th>
                        <th class="num">{{ t('customerLedger.credit') }}</th>
                        <th class="num">{{ t('customerLedger.balance') }}</th>
                    </tr>
                </thead>
                <tbody>
                    <tr class="opening-row">
                        <td colspan="6">{{ t('customerLedger.openingBalance') }}</td>
                        <td class="num">{{ fmtCurrency(report.opening) }}</td>
                    </tr>
                    <tr v-for="(row, i) in report.rows" :key="i">
                        <td>{{ formatDate(row.date) }}</td>
                        <td>
                            <span class="type-pill" :class="typeClass(row.type)">{{ t('customerLedger.type_' + row.type) }}</span>
                        </td>
                        <td class="font-mono text-xs">{{ row.reference || '-' }}</td>
                        <td class="text-slate-600">{{ describe(row) }}</td>
                        <td class="num">{{ row.debit ? fmtCurrency(row.debit) : '' }}</td>
                        <td class="num text-emerald-700">{{ row.credit ? fmtCurrency(row.credit) : '' }}</td>
                        <td class="num font-medium">{{ fmtCurrency(row.balance) }}</td>
                    </tr>
                    <tr v-if="report.rows.length === 0">
                        <td colspan="7" class="py-8 text-center text-slate-400">{{ t('customerLedger.noMovements') }}</td>
                    </tr>
                </tbody>
                <tfoot>
                    <tr>
                        <td colspan="4">{{ t('customerLedger.totals') }}</td>
                        <td class="num">{{ fmtCurrency(report.totals.debit) }}</td>
                        <td class="num text-emerald-700">{{ fmtCurrency(report.totals.credit) }}</td>
...
import {
    PrinterIcon, ArrowPathIcon, DocumentMagnifyingGlassIcon, UserCircleIcon,
...
function formatPeriod(p) {
    const f = p.from ? formatDate(p.from) : '';
    const u = p.to   ? formatDate(p.to)   : '';
    if (!f && !u) return t('common.allTime');
    if (!f) return `${t('common.until')} ${u}`;
    if (!u) return `${t('common.from')} ${f}`;
    return `${f} – ${u}`;
}
...
async function generate() {
    ...
    try {
        ...
        const { data } = await reportService.customerLedger(params);
        report.value = data;
    } catch (err) {
